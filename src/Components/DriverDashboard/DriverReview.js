      ...
      if (user) {
        try {
          const response = await fetch(
            `https://elite-dwell-assist-server.onrender.com/driver?email=${user.email}`
          );
  ...
  useEffect(() => {
    const fetchReviews = async () => {
      if (loggedUser && loggedUser.email) {
        try {
          const response = await fetch(
            `https://elite-dwell-assist-server.onrender.com/review`
          );
          const data = await response.json();
          ...
          if (Array.isArray(data)) {
            ...
            if (driverReviews.length > 0) {
              ...
              try {
                await fetch(
                  "https://elite-dwell-assist-server.onrender.com/averageRatingDriver",
                  {
                    method: "POST",
                    headers: {
