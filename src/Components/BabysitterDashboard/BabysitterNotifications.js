...
const BabysitterNotifications = () => {
  ...
  useEffect(() => {
    fetch("https://elite-dwell-assist-server.onrender.com/babysitterBookings")
      .then((res) => res.json())
      .then((data) => {
        data.map((item) => {
  ...
  useEffect(() => {
    if (user) {
      ...
      fetch(
        `https://elite-dwell-assist-server.onrender.com/babysitterBookings/${loggedInMaidEmail}`
      )
        .then((res) => res.json())
        .then((data) => {
